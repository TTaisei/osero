...
char point_his[SIZE * SIZE - 4][4 + 1] = {0};
...
void think(int num){
    ...
    if (num)
        for (int i = 0; i < 4; i++)
            point_his[num][i] = point_his[num - 1][i];
    for (int i = 0; i < 3; i++)
        point_his[num][i] = point_his[num][i + 1];
    ...
    point_his[num][3] = point[num];
    point_his[num][4] = turn;
}

void print(FILE * fp){
    int my = 3 - PLAYER;

    for (int i = 4; i < SIZE * SIZE - 4; i++){
        if (point_his[i][4] == my){
            // fprintf(fp, "%3d", i);
            for (int j = 0; j < 4; j++)
                fprintf(fp, "%2d", point_his[i][j]);
            fprintf(fp, "\n");
        }
    }
}
